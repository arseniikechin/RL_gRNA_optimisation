...
def read_energy_parameters(ENERGY_MODELS_PICKLE_FILE = "energy_dics.pkl"):
    ...
    loaded = pickle.load(energy_reader)
    energy_reader.close()
    # pkl may be tuple/list (e.g. [dict0, dict1]); pipeline expects dict with keys 0, 1, ...
    if isinstance(loaded, (tuple, list)):
        RNA_DNA = {i: loaded[i] for i in range(len(loaded))}
    else:
        RNA_DNA = loaded
...
def get_eng(grna_seq, off_seq, score_func, GU_allowed=False, pos_weight=False, pam_corr=False, grna_folding=False, dna_opening=False, dna_pos_wgh=False):
    ...
    if pos_weight:
        n_w = min(20, len(scores), len(POS_WGH))
        for i in range(n_w):
    ...
    if dna_opening:
        ...
        if dna_pos_wgh:
            n_w = min(20, len(dna_scores), len(DNA_POS_WGH))
            for i in range(n_w):
